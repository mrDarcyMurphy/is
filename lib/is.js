...
!function (name, definition) {
  ...
  var is = {
    ...
    sameDate: function(x, y) {
      ...
      var z = dryDates(x, y)
      return z.x.y == z.y.y && z.x.m == z.y.m && z.x.d == z.y.d
    },
    ...
    futureDate: function(x, y) {
      if (!is.date(x)) return false
      var z = dryDates(x, y)
      if (z.x.y > z.y.y) return true
      if (z.x.y == z.y.y) {
        if (z.x.m > z.y.m) return true
        if (z.x.m == z.y.m && z.x.d > z.y.d) return true
      }
      return false
    },
  ...
  /**
   * DRY up date methods for slimmer comparisons
   *
   * @method dryDates
   * @param {Date} subject
   * @param {Date} standard, optional
   */
  function dryDates(x, y) {
    y = (!y || !is.date(y)) ? new Date() : y
    return {
      x: { y: x.getFullYear(), m: x.getMonth(), d: x.getDate() },
      y: { y: y.getFullYear(), m: y.getMonth(), d: y.getDate() }
    }
  }
